...
echo "[info:autorenewal] Starting certificate check script..."
...
if [[ -n "$LEGO_RENEW" ]] && [[ "$LEGO_RENEW" = "true" ]]; then
    echo "[warn:autorenewal] LEGO_RENEW should be disabled when using auto-renew. Nothing happens."
    exit
fi
...
end_date_timestamp=$(date -d "$end_date" -D " %B %d %T %Y" +"%s")
end_date_before_expire_timestamp=$(( $end_date_timestamp - $AUTORENEW_PERIOD*24*3600 ))

current_time_timestamp=$(date +%s)

# DEBUG: enable this when testing
# if $DEBUG ; then
#     current_time_timestamp=$(( $current_time_timestamp + 90*24*3600 ))
# fi
...
if [ $current_time_timestamp -lt $end_date_before_expire_timestamp ]; then
